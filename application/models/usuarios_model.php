...
class Usuarios_model extends privilegios_model {
	...
	*/
	...
	/**
	 * Obtiene el listado de usuarios para usar ajax
	 */
	public function getUsuariosAjax($all=true){
		$sql = '';
		$res = $this->db->query("
				SELECT id, nombre, usuario, email 
				FROM usuarios
				WHERE ".($all? '': 'status = 1 AND')." lower(nombre) LIKE '%".mb_strtolower($this->input->get('term'), 'UTF-8')."%'
				ORDER BY nombre ASC
				LIMIT 20");
	...
	*/
	...
	private function crea_session($user_data)
	{
		if (isset($_POST['remember']))
		{
			...
			$this->session->sess_expiration	= 84200;
