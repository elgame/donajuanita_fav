	...
	private function _tag($flag, $ref, $cache = FALSE, $media = 'screen')
	{
		...
		switch($flag){
			...
			case 'css':
				...
				return '<link type="text/css" rel="stylesheet" href="'.$dir.$ref.'?v='.$this->CI->config->item('vcarabiner').'" media="'.$media.'" />'."\r\n";
			...
			case 'js':
				...
				return '<script type="text/javascript" src="'.$dir.$ref.'?v='.$this->CI->config->item('vcarabiner').'" charset="'.$this->CI->config->item('charset').'"></script>'."\r\n";
