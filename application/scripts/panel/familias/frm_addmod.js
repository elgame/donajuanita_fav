$(function(){
	...
	$("#dactualiza_base").on('click', function(){
		var vthis = $(this);
		if($(".tr-produclista").length != 1){
			noty({"text":"Se requiere tener solo 1 producto que consume para actualizar el producto base automaticamente.", "layout":"topRight", "type":"warning"});
			vthis.removeAttr("checked");
			$(".only_update_pbase").addClass('hide');
		}else{
			$(".only_update_pbase").removeClass('hide');
			$("#dprecio_compra").focus();
		}
	});
});
